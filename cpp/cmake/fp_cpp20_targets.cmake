...
function (oop_cpp20_declare_lib BASE_DIR)
    fp_cpp20_dependency_targets()
    # add generated files to library
    add_library(
        oop_cpp20_lib OBJECT
    ...
    target_include_directories(
        oop_cpp20_lib SYSTEM ${warning_guard}
        PUBLIC
        "$<BUILD_INTERFACE:${PROJECT_SOURCE_DIR}/source>"
        ${CMAKE_CURRENT_BINARY_DIR}
    )

    target_compile_features(oop_cpp20_lib PUBLIC cxx_std_20)
    target_link_libraries(oop_cpp20_lib PRIVATE ${FP_CPP20_DEPENDENCY_TARGETS})

endfunction()

function (oop_cpp20_declare_exe BASE_DIR)
    ...
    add_executable(oop_cpp20_exe
        "${BASE_DIR}source/oop/main.cpp"
    )
    add_executable(fp_cpp20::exe ALIAS oop_cpp20_exe)
    set_property(TARGET oop_cpp20_exe PROPERTY OUTPUT_NAME fp_cpp20)
    target_include_directories(
        oop_cpp20_exe SYSTEM ${warning_guard}
        PUBLIC
        "$<BUILD_INTERFACE:${PROJECT_SOURCE_DIR}/source>"
        ${CMAKE_CURRENT_BINARY_DIR}
    )
    target_compile_features(oop_cpp20_exe PRIVATE cxx_std_20)
    target_link_libraries(oop_cpp20_exe PRIVATE oop_cpp20_lib ${FP_CPP20_DEPENDENCY_TARGETS})
endfunction()
...
function (fp_cpp20_declare_lib BASE_DIR)
    ...
    add_library(
        fp_cpp20_lib OBJECT
        "${BASE_DIR}source/fp/parser.cpp"
        "${BASE_DIR}source/fp/parser.h"
        "${BASE_DIR}source/fp/todos.cpp"
        "${BASE_DIR}source/fp/todos.h"
    )
